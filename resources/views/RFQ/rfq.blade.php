    ...
    <table class="table table-bordered" id="myTable" name="myTable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><a href="{{ url('/rfq/data/list/'.$item->id_rfq) }}" class="btn btn-warning" role="button">Edit</a>

                    <form action="{{url('/rfq/delete/'.$item->id_rfq)}}" method="post">
                        @method('delete')
                        {{ csrf_field() }}
                        <button type="submit" onclick="return confirm('Yakin hapus RFQ '+'{{$item->nama_produk}}?');" class="btn btn-danger delete-confirm my-1">
